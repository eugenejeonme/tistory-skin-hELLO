module.exports = class {
  ...
  static sticky(target, context, cls) {
    ...
    if ($context.length) {
      ...
      return () =>
        window.pageYOffset >= contextOffsetTop
          ? $target.addClass(cls)
  ...
  static alignImageCenter(options) {
    ...
    if (options.imageSlideBlock) {
      $("figure.imageslideblock.alignCenter").each(function () {
        alignImage.call(
          this,
          $(this).find(".image-wrap.selected > img").data("origin-width")
        );
      });
    }
  ...
  static anchorHeadings(container, headings) {
    $(container)
      ...
      .each(function () {
        const $heading = $(this);
        const anchor = encodeURIComponent($heading.text()).replace(
          /[0-9-_.!~*'()\s]/g,
          "-"
        );
        const $a = $("<a></a>")
  ...
  static tooltip(messages) {
    ...
    const $reaction = $(
      ".container_postbtn .postbtn_like .wrap_btn[id^=reaction-]"
    );
